...
import { BaseTestRunner, extractParams, unitTestRunner } from "../../tests";
import { BadRequestError } from "../../utils";
import { LIB_CACHE } from "../cache.module";
import { CachePrefixes, redisClient } from ".";

let testRunner: BaseTestRunner;

beforeEach(() => {
  expect.hasAssertions();
  testRunner = unitTestRunner(LIB_CACHE).configure({
    cache: {
...
describe("internals", () => {
  ...
  describe("state", () => {
    // ! RUN THIS TEST FIRST!
    // It is the ONLY ONE that has access to redis before connect
    it("defaults to unhealthy (connecting)", async () => {
      expect.assertions(1);
      const { cache, lifecycle } = await extractParams(testRunner);

      lifecycle.onReady(async () => {
        client = cache.internals.getClient();
        expect(cache.internals.state()).toBe("unhealthy");
      });
    });

    it("recycles connections", async () => {
      expect.assertions(1);
      const { cache } = await extractParams(testRunner.bootLibrariesFirst());
      expect(client).toBe(cache.internals.getClient());
    });

    it("is healthy after doing something", async () => {
      expect.assertions(1);

      await testRunner.run(({ cache, lifecycle }) => {
        lifecycle.onReady(async () => {
          // connecting
          await cache.set("test:3", "asdf");
          expect(cache.internals.state()).toBe("healthy");
        });
      });
    });
  });

  it("count returns an empty array if no keys match the pattern", async () => {
    expect.assertions(1);

    const { cache } = await extractParams(testRunner.bootLibrariesFirst());
    await cache.admin.truncate();
    await cache.set("test:1", "asdf");
    await cache.set("test:2", "asdf");
    await cache.set("test:3", "asdf");
    await cache.set("test:4", "asdf");
    const count = await cache.internals.count("test*");
    expect(count).toBe(4);
  });
});
  ...
  it("should return the default value for a non-existing key", async () => {
    ...
    await testRunner.run(({ cache, lifecycle }) => {
      lifecycle.onReady(async () => {
        ...
        const result = await cache.get(v4(), defaultValue);
...
// #MARK: metrics
describe("metrics", () => {
  it("emits metrics when enabled", async () => {
    expect.assertions(1);
    const id = v4();

    const { cache, metrics } = await extractParams(
      testRunner
        .configure({
          cache: { EMIT_CACHE_STATS: true },
          metrics: { SYSTEM_STATS_INTERVAL: 1 },
        })
        .bootLibrariesFirst(),
    );
    await cache.set(`${CachePrefixes.api_result}:${id}`, "test");
    const spy = jest.spyOn(metrics, "emit");
    await sleep(5);
    expect(spy).toHaveBeenCalledWith("cache-stats", {
      count: expect.any(Number),
      prefix: expect.any(String),
    });
  });

  it("does not emits metrics when disabled", async () => {
    expect.assertions(1);
    const id = v4();

    const { cache, metrics } = await extractParams(
      testRunner
        .configure({ cache: { EMIT_CACHE_STATS: false } })
        .bootLibrariesFirst(),
    );
    await cache.set(`${CachePrefixes.api_result}:${id}`, "test");
    const spy = jest.spyOn(metrics, "emit");
    await sleep(5);
    expect(spy).not.toHaveBeenCalledWith("cache-stats", {
      count: expect.any(Number),
      prefix: expect.any(String),
    });
  });
...
describe("admin", () => {
  describe("purgeByType", () => {
    it("works", async () => {
      ...
      await testRunner.bootLibrariesFirst().run(async ({ cache }) => {
        await cache.set(`${CachePrefixes.api_result}:${id}`, "test");
        const count = await cache.internals.count(
          `${CachePrefixes.api_result}:*`,
        );
        expect(count).not.toBe(0);
        await cache.admin.purgeByType(CachePrefixes.api_result);
        const final = await cache.internals.count(
          `${CachePrefixes.api_result}:*`,
        );
        expect(final).toBe(0);
      });
    ...
    it("checks type", async () => {
      ...
      await testRunner.bootLibrariesFirst().run(async ({ cache }) => {
        try {
          await cache.admin.purgeByType("error");
        } catch (error) {
