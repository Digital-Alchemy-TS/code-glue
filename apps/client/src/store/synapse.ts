...
const synapseFactory = createFactory<SynapseEntities>({
	...
	.actions({
		push() {
			return fetch(`${baseUrl}/api/v1/synapse/${this.id}`, {
				method: "PUT",
				headers: {
				...
				.then((json: SynapseEntities) => {
					Object.entries(json).forEach(([key, value]) => {
						this[key as keyof SynapseEntities] = value as never
					})
				})
				.catch((error) => {
...
export const createSynapseEntity = (
	initialData: SynapseEntityCreateOptions,
) => {
